...
static void enforceUniqueSuccessor(const ScopeGraphBuilder &SGBuilder,
                                   BasicBlock *Head,
...
static void verifyHeadMD(BasicBlock *Head) {
  ...
  // We check that the metadata contains the expected tag
  Function *F = Head->getParent();
  QuickMetadata QMD(getContext(F));
  auto HeadMD = QMD.extract<StringRef>(Tuple, 0);
  revng_assert(HeadMD == "genericregion-head");
}
...
collectExitBlocks(GenericRegion<BasicBlock *> *Region,
                  SmallSet<BasicBlock *, 4> &ExitingBlocks,
                  SmallSet<BasicBlock *, 4> &UniqueSuccessorCandidates) {
...
/// Helper function used to elect the `GenericRegion` successor when a `Footer`
/// is needed
static BasicBlock *
  ...
  bool run() {
    ...
    for (auto &TopLevelRegion : RegionInfo.top_level_regions()) {
      for (auto *Region : post_order(&TopLevelRegion)) {
        ...
        // 2. We try to identify the `UniqueSuccessor` in different phases:
        ...
        if (UniqueSuccessorCandidates.size() == 1) {
          ...
          enforceUniqueSuccessor(SGBuilder, Head, *UniqueSuccessor);

          // We mark the current `Function` as modified
          FunctionModified = true;

          // We already inserted the `scope_closer`, so we can skip to the next
          // `GenericRegion`
          continue;
        }
        ...
        if (ExitingBlocks.size() == 1
            and UniqueSuccessorCandidates.size() > 1) {
          BasicBlock
            *Footer = enforceFooterSuccessor(Region,
                                             ExitingBlocks,
                                             UniqueSuccessorCandidates);

          // We insert the `scope_closer` to the `footer`
          enforceUniqueSuccessor(SGBuilder, Head, Footer);
        ...
        // If we did not find a `UniqueSuccessor`, we move to the next
        // `GenericRegion`
        ...
        if (ParentRegion) {
          ...
          // If the preconditions are not satisfied, we move to the next
          // `GenericRegion`
